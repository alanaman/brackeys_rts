...
//You can transform the number on screen to the stat here, using the function at the bottom of this script
    ...
    [SerializeField, Range(0.2f, 1.25f)][Tooltip("How long it takes to reach that height before coming back down")] public float timeToJumpApex = 0.4f;
    ...
    private void setPhysics()
    {
        ...
        Vector2 newGravity = new Vector3(0, (-2 * jumpHeight) / (timeToJumpApex * timeToJumpApex), 0);
        gravity.gravityScale = (newGravity.y / Physics.gravity.y) * gravMultiplier;
    }
    ...
    private void DoAJump()
    {
        ...
        if (onGround || (coyoteTimeCounter > 0.03f && coyoteTimeCounter < coyoteTime) || canJumpAgain)
        {
            ...
            jumpSpeed = Mathf.Sqrt(-2f * Physics.gravity.y * gravity.gravityScale * jumpHeight);
    ...
    public void bounceUp(float bounceAmount)
    {
        ...
        rb.AddForce(Vector2.up * bounceAmount, ForceMode.Impulse);
    }
}